...
const ComponentInvocation = class {
     ...
     */
     channelType() {
         return this.request().message.channel.type;
     }
     ...
     * It is not possible to create new variables or change the type of an
     * existing variable through this method.  It is the caller's responsibility
     * to ensure that the value being set on a variable is of the correct type.
     * (e.g. entity, string or other primitive, etc).
     *
     ...
     */
    keepTurn(k) {
      this._response.exit = (typeof k !== 'undefined' ? !k : true);
        return this;
    }
    /**
    ...
    reply(payload) {
        var response = {
          ...
          channel: Object.assign({}, this._request.message.channel)
        };
...
function validateRequestBody(reqBody) {
    ...
    const propertiesSchema = Joi.object().pattern(/(.*)/,
        Joi.alternatives().try([Joi.string().allow(''), Joi.number(), Joi.boolean(), Joi.array()]));
    ...
    return requestSchema.validate(reqBody);
}
